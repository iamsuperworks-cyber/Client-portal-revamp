...
const SigningModal = ({
  ...
  /* ─── Mobile document stepper (replaces sidebar) ─── */
  const MobileDocStepper = () => (
    <div className="md:hidden flex items-center gap-1.5 px-4 py-2.5 border-b border-border bg-secondary overflow-x-auto shrink-0">
      {documents.map((doc, idx) => {
        ...
        return (
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-[10px] font-medium whitespace-nowrap shrink-0 transition-colors ${
              isActive
          ...
          >
            <div
              className={`w-3.5 h-3.5 rounded-full shrink-0 flex items-center justify-center ${
                isSigned ? "bg-primary" : isActive ? "border-2 border-primary" : "border border-border"
              }`}
            >
              {isSigned && <Check className="h-2 w-2 text-primary-foreground" />}
            </div>
            {doc.name}
          </button>
        );
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end md:items-center justify-center">
      ...
      <div className="relative z-10 bg-card shadow-xl flex flex-col overflow-hidden w-full h-full md:rounded-lg md:w-[90vw] md:max-w-[1180px] md:h-[85vh] md:max-h-[680px]">

        {/* Header */}
        <div className="flex items-center px-4 md:px-5 border-b border-border shrink-0 h-12 md:h-12">
          <div className="flex-1" />
          <div className="flex-1 flex items-center justify-center">
            <p className="font-heading text-xs md:text-sm font-semibold text-foreground text-center truncate">
              {bankName} — Documents to Sign
            </p>
          </div>
        ...
        {/* Mobile doc stepper */}
        {!showSignatureUpload && !allComplete && <MobileDocStepper />}
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <div
            className={`hidden md:flex shrink-0 bg-secondary border-r border-border flex-col transition-opacity duration-300 w-[200px] ${
              showSignatureUpload ? "opacity-40 pointer-events-none" : ""
            }`}
          >
          ...
          <div className="flex-1 flex flex-col min-h-0 relative">
            {showSignatureUpload ? (
              <div className="flex-1 flex items-center justify-center bg-muted/30 p-4 md:p-6 overflow-y-auto">
                <div className="bg-card rounded-lg border border-border p-5 md:p-8 max-w-[420px] w-full shadow-sm">
                  <h2 className="font-heading text-base md:text-lg font-bold text-foreground mb-2">Set your signature</h2>
                  <p className="font-body text-xs text-muted-foreground mb-5 md:mb-6 leading-relaxed">
                  ...
                  <div
                    ...
                    className={`border border-dashed border-border rounded-lg flex flex-col items-center justify-center mb-2 transition-colors relative overflow-hidden ${
                      !signaturePreview ? "p-6 md:p-8 cursor-pointer hover:bg-muted/50" : "h-[120px] md:h-[160px]"
                    }`}
                  >
                    {signaturePreview ? (
                      <img
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="font-body text-[10px] text-muted-foreground mt-0.5">Removes white or light backgrounds</p>
                          </div>
                        </div>
                        <button
                          onClick={() => fileInputRef.current?.click()}
                          className="font-body text-[10px] text-muted-foreground hover:text-foreground underline underline-offset-2 transition-colors mt-0.5 shrink-0"
                        >
                          Replace
                        </button>
                      </div>
                  ...
                  <button
                    ...
                    className={`w-full mt-5 md:mt-6 font-body text-[11px] font-semibold cursor-pointer rounded-sm px-6 py-2.5 transition-colors ${
                      signaturePreview
              ...
              <>
                ...
                <div className="md:hidden flex items-center gap-2 px-4 py-2 border-b border-border bg-card shrink-0">
                  ...
                  <span className="font-body text-[10px] text-muted-foreground whitespace-nowrap">
                    {currentDocSignedFields} of {activeDoc?.signatureCount} signed
                  </span>
                </div>

                {/* Document content */}
                <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-muted/30">
                  <div className="bg-card rounded-lg border border-border p-5 md:p-8 max-w-2xl mx-auto relative shadow-sm">
                    <h3 className="font-heading text-sm md:text-base font-bold text-foreground mb-3 md:mb-4">{activeDoc?.name}</h3>
                    {PLACEHOLDER_PARAGRAPHS.map((p, i) => (
                      <p key={i} className="font-body text-[11px] md:text-xs text-foreground/80 leading-relaxed mb-2.5 md:mb-3">
                        {p}
                      </p>
                    ))}
                    <div className="mt-6 md:mt-10 pt-4 md:pt-6 border-t border-border space-y-3 md:space-y-4">
                      {Array.from({ length: activeDoc?.signatureCount || 0 }).map((_, i) => {
                        const isSigned = i < currentDocSignedFields;
                        return (
                          <div
                            key={i}
                            className={`rounded border-2 border-dashed flex flex-col items-center justify-center transition-colors p-2.5 md:p-3 ${
                          ...
                          >
                            {isSigned ? (
                              ...
                              <>
                                ...
                                <span className="font-body text-[10px] text-primary/70 mt-0.5 text-center">
                                  Tap Insert Signature below
                                </span>
                              </>
                            )}
                          </div>
                ...
                {/* Footer */}
                <div className="border-t border-border px-3 md:px-6 py-2.5 md:py-3 flex items-center justify-between bg-card shrink-0 gap-2">
                  <button
                    onClick={handlePrev}
                    disabled={activeDocIndex === 0}
                    className="flex items-center justify-center font-body text-[11px] font-semibold cursor-pointer rounded-sm px-2.5 md:px-3 py-1.5 border border-border text-foreground hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                  >
                    <ChevronLeft className="h-3.5 w-3.5 mr-0.5 md:mr-1" />
                    Prev
                  </button>
                  <button
                    onClick={handleInsertSignature}
                    disabled={isCurrentDocFullySigned}
                    className="flex items-center gap-1.5 font-body text-[11px] font-semibold cursor-pointer rounded-sm px-4 py-2 bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                  >
                  ...
                  >
                    ...
                    {!(activeDocIndex === documents.length - 1 && isCurrentDocFullySigned) && <ChevronRight className="h-3.5 w-3.5 ml-0.5 md:ml-1" />}
                  </button>
                </div>
              </>
            ) : (
              /* Completion screen */
              <div className="flex-1 flex flex-col items-center justify-center gap-3 md:gap-4 px-6 md:px-8">
                <div className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-primary flex items-center justify-center">
                  <Check className="h-6 w-6 md:h-7 md:w-7 text-primary-foreground" />
                </div>
                {hasPendingOtherBanks ? (
                  <>
                    <h2 className="font-heading text-lg md:text-xl font-bold text-foreground text-center">All {bankName} forms signed</h2>
                    <p className="font-body text-xs md:text-sm text-muted-foreground text-center max-w-md">
                      You still have pending forms with {pendingOtherBankNames.join(" and ")}.
                    </p>
                    <button
                      onClick={onContinueSigning}
                      className="mt-3 md:mt-4 w-full max-w-xs font-body text-[11px] font-semibold cursor-pointer rounded-sm px-6 py-2.5 bg-primary text-primary-foreground hover:bg-primary/90 transition-colors"
                    >
                      Sign Next Bank
                    </button>
                    <button
                  ...
                  <>
                    <h2 className="font-heading text-lg md:text-xl font-bold text-foreground text-center">All forms signed</h2>
                    <p className="font-body text-xs md:text-sm text-muted-foreground text-center max-w-md">
